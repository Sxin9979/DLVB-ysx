...
class VBinfo_collector:
    ...
    def match_orbital_data(self, orbital_content): # 根据轨道匹配轨道对应的原子
        """ 计算每条轨道上的原子系数之和 """
    ...
    def read_geo_from_out_files(self, file_path, VBdata): # 从输出文件读取信息
        ...
        for line in lines:
            ...
            if "******  ORBITALS IN PRIMITIVE BASIS FUNCTIONS ******" in line :
                ...
                continue
            elif "******   COMPUTED NATURAL ORBITALS   ******" in line and in_orbital_section:
                break            
            elif in_orbital_section and line :
    ...
    def save_to_file(self, output_path="/pool1/home/ysxin/E3nnVB/VB/vb_data_collection.pt"):
        """将处理好的数据保存到文件"""
        data_to_save = {
