...
def covalent_radii(coor,nodes,atom_nums): # 生成共价半径和距离矩阵
    radius = {
...
# def gen_relative_coor(coor_)

def match_orbital_data(orbital_content): # 根据轨道匹配轨道对应的原子
    """ 计算每条轨道上的原子系数之和 """
    ...
    for orbital_idx, orbital_data in enumerate(all_orbitals):
        ...
        sorted_atoms = sorted(orbital_data.items(), 
...
def orb_match_atom(orbital_matched): 
    """ 根据每条轨道上的原子系数之和，将轨道与原子相配对 """
    atom_to_number = atom_map()
    result = []    
...
# 通过RDkit得到成键方式
def tensor_to_xyz_block(atoms, coords):
...
def get_bond_info(atoms, coords):
    """根据原子坐标自动判断所有成键信息"""
    xyz_block = tensor_to_xyz_block(atoms, coords)
...
def str_to_Amat(nodes, atom_from_orb, str, coor, atom_nums, sym): 
    """ 生成邻接矩阵 """
    ...
    for idx, parts in enumerate(str):
        # 首先判断该分子的活性部分是否具有共轭性
        if_cov=0
        for chars in parts:
            if "-" in chars: 
                orb_1, orb_2 = chars.split('-')
                atom_1=atom_from_orb[int(orb_1)-1, 2]
                atom_2=atom_from_orb[int(orb_2)-1, 2]
        ...
        for i in range(nodes):
            for j in range(nodes):
                if Amat[str_idx][i][j]:
                    ...
                    E[0].append(2 * A_BO_act[str_idx][i][j].item())
                    E[1].append(2 * A_BO_inact[i][j].item())

    print("A_list, E \n",A_list,"\n",E)

#     # 邻接矩阵转化成邻接列表
#     A_list = [
#     [i+1 for k in range(len(str)) for i in range(nodes) for j in range(nodes) if Amat[k][i][j] != 0],
#     [j+1 for k in range(len(str)) for i in range(nodes) for j in range(nodes) if Amat[k][i][j] != 0]
# ] 
...
#                 if Amat[str_idx][i][j] :
#                     E[0].append(2*A_BO_act[str_idx][i][j]),E[1].append(2*A_BO_inact[str_idx][i][j])




    # 键级矩阵s
    # print("Amat, Alist, A_BO_act, A_BO_inact\n",Amat, A_list, A_BO_act, A_BO_inact)
...
def read_geo_from_out_files(VBdata): # 从输出文件读取信息
    ...
    for file_name in out_files:
        ...
        for line in lines:
            ...
            if line=="$geo" :
                if_in_geo = True
            elif line=="$end" and if_in_geo:
                break
            elif if_in_geo and line:
    ...
    for file_name in out_files:
        ...
        for line in lines:
            ...
            if "Lowdin Weights" in line:
                if_in_weight = True
            elif "Inverse Weights" in line and if_in_weight:
                break
            elif if_in_weight and line:
                weights = line.split()
                for i, part in enumerate(weights):
                    if '1:' in part:
                        weight_data.append(weights[i:])
                        break

        VBdata.str=weight_data

    # 3、处理轨道部分，得到轨道对应的原子
    for file_name in out_files:
        with open(file_name,'r',encoding='utf-8') as file:
            lines = file.readlines()
            if_in_orbitals = False
            orbital_data=[]

        for line in lines:
            line=line.strip()
            if "******  ORBITALS IN PRIMITIVE BASIS FUNCTIONS ******" in line :
                if_in_orbitals = True
            elif "******   COMPUTED NATURAL ORBITALS   ******" in line and if_in_orbitals:
                break            
            elif if_in_orbitals and line :
                orbital_data.append(line)
          
        orbital_matched=match_orbital_data(orbital_data) # 匹配轨道对应的原子
        
        VBdata.atom_from_orb = orb_match_atom(orbital_matched).clone().detach()
...
def generate_A(VBdata): # 生成邻接矩阵
    # Amat=str_to_Amat(VBdata.nodes, VBdata.atom_from_orb, VBdata.str, VBdata.coor,VBdata.atom_nums,VBdata.sym)
    print(VBdata.str)
    num_graph=len(VBdata.str) # 每个分子的结构数，即这个分子的图的数量

    Amat=torch.zeros(num_graph, VBdata.nodes, VBdata.nodes, dtype=torch.int)
    A_BO_inact=torch.zeros(VBdata.nodes, VBdata.nodes, dtype=torch.int)    # 所有的结构，A_BO_inact都是一样的
    A_BO_act=torch.zeros(num_graph, VBdata.nodes, VBdata.nodes, dtype=torch.int)
    BO_if_changed=torch.zeros(VBdata.nodes, VBdata.nodes, dtype=torch.int) # 用来判断该非活性键级是否修改过，避免累计
    
    # 得到一个通过RDkit形成的键级矩阵，非活性部分解决
    bonds=get_bond_info(VBdata.sym, VBdata.coor)
    print("bonds\n",bonds)
    for parts in bonds:  
        # print(parts[1]-1," ",parts[3]-1, " + ", parts[5])
        A_BO_inact[parts[1]-1][parts[3]-1]=A_BO_inact[parts[3]-1][parts[1]-1]=parts[5]
...
if __name__ == "__main__":
    ...
    VBdata=read_geo_from_out_files(VBdata)
    VBdata=generate_A(VBdata)
    VBdata=generate_X(VBdata)
