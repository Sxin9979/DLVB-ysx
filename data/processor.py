...
class XmoDatasetProcessor:
    ...
    def discoverXmoFiles(self) -> list[Path]:
        """Return sorted ``.xmo`` files from the configured input directory."""

        xmo_dir = Path(self.config.xmo_dir)
        if not xmo_dir.exists():
            raise FileNotFoundError(f"Configured xmo_dir does not exist: {xmo_dir}")
        files = sorted(xmo_dir.glob("*.xmo"))
        if len(files) == 0:
            raise FileNotFoundError(f"No .xmo files were found under {xmo_dir}")
        return files
    ...
    def createMoleculeSplits(self, molecule_ids: list[str]) -> Dict[str, List[str]]:
        ...
        ratio = np.asarray(self.config.split, dtype=np.float64)
    ...
    def buildProcessedMolecule(self, built: BuiltMoleculeSample) -> ProcessedMolecule:
    ...
    def buildProcessedDatasetCache(self) -> ProcessedDatasetCache:
        ...
        for path in self.discoverXmoFiles():
            try:
                parsed = self.parser_cls(path).parse()
                built = self.builder.build(parsed)
                processed = self.buildProcessedMolecule(built)
        ...
        split_ids = self.createMoleculeSplits(sorted(processed_molecules.keys()))
        return ProcessedDatasetCache(split_ids=split_ids, molecules=processed_molecules)
    ...
    def buildPackedDatasetCache(self) -> PackedDatasetCache:
        """Pack processed molecules into the runtime-efficient single-molecule chunk cache."""
